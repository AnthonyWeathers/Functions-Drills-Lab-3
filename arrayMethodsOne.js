...
// CODE HERE
const evenNumbers = mixedNumbers.filter(evenNumber => evenNumber % 2 === 0)/* Provide Your Callback Here */

console.log("An array with only even numbers from the mixedNumbers array: " + evenNumbers)

// evenNumbers = [6, 2, 6, 8, 4, 2, 8, 4]
...
// CODE HERE
const postTaxPrices = prices.map(newPrice => newPrice * 1.07)// Provide Your Callback Here );

console.log("The prices after tax: " + postTaxPrices)
...
// CODE HERE
const totalPopulation  = populations.reduce((sum, currentIndex) => sum + currentIndex) //(/* Provide Your Callback Here */)

console.log("Total population: " + totalPopulation)

// totalPopulation = 16763615
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(strongMonsters => strongMonsters.CP >= 200) // (/* Provide Your Callback Here */)

console.log("Monsters with over 200 CP: " + myStrongest)
...
const orderTotal = orders.map((elem, i) => {
  let taxPrice = 1
  for (value in elem) {
        taxPrice *= orders[i][value] 
      } // maybe could have done let taxPrice = orders[i]['price'] * orders[i]['tax'] or orders[i].price * orders[i].tax
  return orders[i]['price'] + taxPrice
})
console.log("Official price after tax: " + orderTotal)
...
// CODE HERE
bobsTotal = purchases.reduce((total, currPurchase) => {
  //console.log(`'The current purchases is ${currPurchase['owner']}`)
  if (currPurchase['owner'] === 'Bob') {
    //console.log(currPurchase['price'])
    return total += currPurchase['price']
  } else {
    return total += 0
  }
}, 0)

console.log("Total cost for Bob's purchases: " + bobsTotal)
